...
describe("derived role patterns", () => {
  ...
  function makeResolver(overrides: {
    ...
    return {
      getRoles: async (actor: ActorRef, res: ResourceRef) => {
        const key = `${actor.type}:${actor.id}:${res.type}:${res.id}`;
        return overrides.roles?.[key] ?? [];
      },
      getRelated: async (res: ResourceRef, relationName: string) => {
        const key = `${res.type}:${res.id}:${relationName}`;
        return overrides.related?.[key] ?? [];
      },
      getAttributes: async (ref: ResourceRef) => {
        const key = `${ref.type}:${ref.id}`;
        return overrides.attributes?.[key] ?? {};
      },
    };
  }
  ...
  describe("Pattern 1: Global Role (from_global_role)", () => {
    ...
    it("derives role when actor matches global role conditions", async () => {
      ...
      const result = await resolveRoles(actor, resource, resolver, projectBlock, policy);
  ...
  describe("Pattern 2: Relation-based (from_role + on_relation)", () => {
    ...
    const projectBlock = policy.resources["Project"]!;
    const orgRef: ResourceRef = { type: "Organization", id: "org1" };

    it("derives role when actor has the required role on the related resource", async () => {
      const resolver = makeResolver({
        related: { "Project:p1:org": orgRef },
        roles: { "User:u1:Organization:org1": ["admin"] },
      });
      const result = await resolveRoles(userActor, resource, resolver, projectBlock, policy);
    ...
    it("does not derive role when actor lacks the required role on related resource", async () => {
      const resolver = makeResolver({
        related: { "Project:p1:org": orgRef },
        roles: { "User:u1:Organization:org1": ["member"] },
      });
      const result = await resolveRoles(userActor, resource, resolver, projectBlock, policy);
  ...
  describe("Pattern 3: Relation Identity (from_relation)", () => {
    ...
    it("derives role when actor ID matches the relation target", async () => {
      const resolver = makeResolver({
        related: { "Task:t1:assignee": { type: "User", id: "u1" } },
      });
      const result = await resolveRoles(userActor, taskRef, resolver, taskBlock, policy);
    ...
    it("does not derive role when actor ID does not match", async () => {
      const resolver = makeResolver({
        related: { "Task:t1:assignee": { type: "User", id: "u999" } },
      });
      const result = await resolveRoles(userActor, taskRef, resolver, taskBlock, policy);
    ...
    it("does not derive role when actor type does not match", async () => {
      ...
      // The policy only declares User actor, but the relation target type is User
      // A ServiceAccount with same ID should not match because types differ
      const policyWithSA: Policy = {
        ...policy,
        actors: { ...policy.actors, ServiceAccount: { attributes: {} } },
      };
      const result = await resolveRoles(saActor, taskRef, resolver, taskBlock, policyWithSA);

      expect(result.derived.some(d => d.role === "editor")).toBe(false);
    });

    it("derives role when actor matches one of many relation targets", async () => {
      const manyPolicy: Policy = {
        ...
        resources: {
          ...
          Task: {
            ...taskBlock,
            relations: {
              assignee: { resource: "User", cardinality: "many" },
            },
          },
        },
      };
  ...
  describe("Pattern 4: Actor-type + when condition", () => {
    ...
    it("derives role when actor type matches and condition is met", async () => {
      ...
      const resolver = makeResolver({});
      const result = await resolveRoles(actor, docRef, resolver, docBlock, policy);
  ...
  describe("Pattern 5: when-only condition", () => {
    ...
    it("derives role when condition is met (any actor type)", async () => {
      ...
      const resolver = makeResolver({});
      const result = await resolveRoles(actor, reportRef, resolver, reportBlock, policy);
  ...
  describe("Exhaustive evaluation", () => {
    it("evaluates all derived role entries even after a match", async () => {
      ...
      const result = await resolveRoles(actor, taskRef, resolver, taskBlock, policy);
